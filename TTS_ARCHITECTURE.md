...
教学请求要按资产类型区分：词组/短语正文只放真正要朗读的粤语文本，读音约束优先放进 `pronunciation_dict`；单字不能直接把孤字交给 MiniMax，自建受控素材（如目标字重复三次）并用时间轴取中间目标字，生成干净 `char_audio`。
...
1. 保留现有整句预处理，用于句子播放。
2. 新增独立 `char_audio` / `word_audio`：词组用 `pronunciation_dict`，单字用受控素材生成并裁出目标字，调试日志记录 `pronunciationMode` 和队列状态。
